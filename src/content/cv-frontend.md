...
<!--
  Fonte do CV renderizado — VISÃO FRONTEND. O par deste arquivo é cv.md (visão fullstack):
  mesmo conteúdo, outra ordem e outra frase de especialização no Summary.
  ...
  - "- " → bullet. A página exibe SÓ OS 6 PRIMEIROS bullets de cada experiência;
    os demais ficam aqui como banco, em ordem de relevância para o cargo frontend.
  ...
  Diferenças em relação ao cv.md, todas registradas na receita "Versão frontend" do mestre:
  - title: Frontend Engineer (o cv.md usa Full-Stack Engineer).
  - Summary: abertura frontend + frase de especialização frontend. Título e abertura andam
    juntos — trocar um sem o outro deixa cabeçalho e primeira linha se contradizendo.
  - Bytes & Pixels: o MCP entra na redação [F] (widgets React + build Vite), não na [F/B].
  - Os bullets de acessibilidade (Mais Retorno) e PostHog (Bytes) sobem para os 6 visíveis.
  - Core Expertise usa a lista frontend; Technical Skills sobe Acessibilidade.
-->

## Summary

Frontend Engineer with 6+ years of experience building scalable, high-performance web interfaces — from React and Next.js applications to the Node.js and NestJS services behind them, with hands-on experience in payments, authentication, background processing, and search systems. Deep specialization in Next.js rendering strategies (SSR, SSG, App Router), frontend performance optimization, component architecture with TypeScript and Tailwind CSS, automated testing with Playwright and Jest, and application observability with PostHog and Sentry.
